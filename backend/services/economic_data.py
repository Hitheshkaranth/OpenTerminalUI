"""Economic terminal data service.

Provider chain (first non-empty result wins, results are cached):

  calendar:   Finnhub (key) -> FMP (key) -> FXMacroData (free, USD) -> mock
  indicators: FRED (key, US/IN/EU/CN) merged with FXMacroData (free, US) -> mock

FXMacroData needs no API key, so a fresh install gets real US releases and
observations without any configuration. Keyed providers, when configured, add
actual/forecast values and non-US regions on top.
"""
...
from backend.services.fxmacrodata_economics import FXMacroDataEconomics, OperationResult
...
# FXMacroData indicator ids -> MACRO_CONFIG labels, so the US panel has one entry per concept.
FXMACRODATA_TO_FRED_LABEL = {
    "policy_rate": "rate",
    "inflation": "cpi",
...
class EconomicDataService:
    def __init__(self, fxmacrodata: FXMacroDataEconomics | None = None):
        ...
        self.fxmacrodata = fxmacrodata or FXMacroDataEconomics()

    # ------------------------------------------------------------------ #
    # FXMacroData passthrough (Data Explorer)
    # ------------------------------------------------------------------ #
    def operations(self) -> List[Dict[str, Any]]:
        return self.fxmacrodata.operations()

    async def query(self, operation: str, arguments: Dict[str, Any]) -> OperationResult:
        return await self.fxmacrodata.query(operation, arguments)
    ...
    async def get_economic_calendar(self, start_date: str, end_date: str) -> List[Dict[str, Any]]:
        ...
        # Free tier: FXMacroData public USD release calendar (no key required)
        if not events:
            try:
                events = await self.fxmacrodata.get_economic_calendar(start_date, end_date)
            except ValueError:
                raise
            except Exception as e:
                logger.error(f"FXMacroData calendar error: {e}")
                events = []

        # Mock data if every provider failed or keys missing
        if not events:
    ...
    async def get_macro_indicators(self) -> Dict[str, Any]:
        ...
        # Free tier: FXMacroData public USD observations fill any US gaps.
        try:
            fx = await self.fxmacrodata.get_macro_indicators()
        except Exception as e:
            logger.error(f"FXMacroData indicators error: {e}")
            fx = {}
        us_fx = fx.get("us") or {}
        if us_fx:
            us = results.setdefault("us", {})
            for key, value in us_fx.items():
                us.setdefault(FXMACRODATA_TO_FRED_LABEL.get(key, key), value)
