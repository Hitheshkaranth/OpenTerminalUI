    ...
    return (
        <div className="rounded border border-terminal-border bg-terminal-panel p-4">
            ...
            <div className="space-y-4">
                {/* Event dates are date-only ("YYYY-MM-DD" → UTC midnight); read them in UTC so
                    users west of UTC don't see the previous day. */}
                {data && data.map((evt: MarketEvent, idx: number) => (
                    <div key={idx} className="flex items-start space-x-3 border-l-2 border-terminal-accent pl-3">
                        <div className="flex-shrink-0 w-12 text-center">
                            <div className="text-xs uppercase text-terminal-muted">{new Date(evt.date).toLocaleString("default", { month: "short", timeZone: "UTC" })}</div>
                            <div className="text-xl font-bold text-terminal-text">{new Date(evt.date).getUTCDate()}</div>
                        </div>
                        <div>
