...
const STRATEGY_INDICATORS: Record<string, IndicatorConfig[]> = {
  "example:sma_crossover": [
    { id: "sma", params: { period: 20 }, visible: true, color: "#00c176", lineWidth: 2 },
    { id: "rsi", params: { period: 14 }, visible: true, color: "#ffb74d", lineWidth: 1 },
  ],
  "example:ema_crossover": [
    { id: "ema", params: { period: 12 }, visible: true, color: "#5aa9ff", lineWidth: 2 },
    { id: "macd", params: { fastPeriod: 12, slowPeriod: 26, signalPeriod: 9 }, visible: true, color: "#d8dde7", lineWidth: 1 },
  ],
  "example:mean_reversion": [
    { id: "bb", params: { period: 20, stdDev: 2 }, visible: true, color: "#ab47bc", lineWidth: 1 },
    { id: "rsi", params: { period: 14 }, visible: true, color: "#ffb74d", lineWidth: 1 },
  ],
  "example:breakout_20": [
    { id: "donchian", params: { period: 20 }, visible: true, color: "#26a69a", lineWidth: 1 },
    { id: "atr", params: { period: 14 }, visible: true, color: "#ef5350", lineWidth: 1 },
  ],
};
...
export function BacktestingPage() {
  ...
  const [symbol, setSymbol] = useState((storeTicker || "RELIANCE").toUpperCase());
  ...
  useEffect(() => {
    if (storeTicker) {
      setSymbol(storeTicker.toUpperCase());
    }
  }, [storeTicker]);
  ...
  }, [jobState, script, strategyMode, symbol]);
  ...
  return (
    <div className="h-full space-y-3 overflow-y-auto px-3 py-2 pb-4">
        ...
        <TerminalPanel title="Backtesting Control Deck" subtitle="Compact controls for chart-first workflow">
          <div className="grid grid-cols-1 gap-2 text-xs md:grid-cols-6">
            <label className="md:col-span-1">
              <span className="mb-1 block text-[11px] uppercase tracking-wide text-terminal-muted">Symbol</span>
              <input
          ...
          {strategyMode === CUSTOM_STRATEGY_VALUE && (
            <label className="mt-2 block">
              ...
              <textarea
                className="h-36 w-full resize-none rounded border border-terminal-border bg-[#040506] px-2 py-1 font-mono text-[11px] text-terminal-text"
                value={script}
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-[1fr_320px]">
        <TerminalPanel title="Underlying Price with Buy/Sell Events" subtitle={`${symbol.toUpperCase()} ${market}`}>
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
        ...
        <TerminalPanel title="Execution Logs" subtitle="Strategy stdout/stderr">
          <pre className="max-h-56 overflow-auto whitespace-pre-wrap bg-[#040506] p-2 font-mono text-[11px] text-terminal-muted">
            {result?.logs || "No logs"}
          </pre>
        </TerminalPanel>
      </div>
