...
def compute_ratios(row: dict[str, Any]) -> dict[str, Any]:
    """Computes core valuation, profitability, leverage and growth metrics."""
    out = dict(row)
    ...
    out["op_margin_pct"] = (
        float(row.get("operating_margin")) * 100
        if isinstance(row.get("operating_margin"), (int, float))
        else None
    )
    out["net_margin_pct"] = (
        float(row.get("profit_margin")) * 100
        if isinstance(row.get("profit_margin"), (int, float))
        else None
    )
    out["rev_growth_pct"] = (
        float(row.get("revenue_growth")) * 100
        if isinstance(row.get("revenue_growth"), (int, float))
        else None
    )
    out["eps_growth_pct"] = (
        float(row.get("earnings_growth")) * 100
        if isinstance(row.get("earnings_growth"), (int, float))
        else None
    )
    out["div_yield_pct"] = (
        float(row.get("dividend_yield")) * 100
        if isinstance(row.get("dividend_yield"), (int, float))
        else None
    )
    return out
